 ...
 * HCL Commerce
 *
 * (C) Copyright HCL America, Inc. Limited 2021
 *
 *---------------------------------------------------
 */
//Standard libraries
...
import storeUtil from "../../utils/storeUtil";
import bVService from "../bv/apis/bv.service";
import { Reviews } from "@hcl-commerce-store-sdk/react-component";
import {Questions} from "@hcl-commerce-store-sdk/react-component"
...
const useProductDetails = (page: Page) => {
  ...
  const [ratingSummaryContent,setRatingSummaryContent]=useState<string>('');
  const [reviewsContent,setReviewsContent]=useState<string>('');
  ...
  React.useEffect(() => {
    if (currentSelection?.sku[currentSelection?.index]?.id) {
      ...
      //bazarvoicecontent render code.
      bVService.getRatingAndReviews(currentSelection?.sku[currentSelection?.index]?.id).then(response=>{
        setRatingSummaryContent(response.data?.ratingSummary);
        setQuestionsContent(response.data?.questions);
        setReviewsContent(response.data?.reviews)
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  if (displayLongDesc) {
    ...
    const reviewsElement = (
      <>
       <Reviews content={reviewsContent} productId={product.id}></Reviews>
      </>
    );
    const questionsElement = (
      <>
        <Questions content={questionsContent} productId={product.id}></Questions>
      </>
    );
    ...
    {
      title: "Reviews",
      tabContent: reviewsElement,
    },
    {
      title: "Questions",
      tabContent: questionsElement,
    }
