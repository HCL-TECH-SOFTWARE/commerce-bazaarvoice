...
export const useProductGridLayout = (props: any) => {
  ...
  const _productListByImageSearch = useSelector(productListByImageSelector);
  ...
  useEffect(() => {
    if (mySite && contract !== null && (!isEmpty(categoryId) || !isEmpty(searchTerm))) {
      ...
      if (parameters.searchTerm && parameters.searchTerm==='IMAGESEARCH') {

        const productArr = _productListByImageSearch.map((productobj) => {
          const labelArr = productobj.product.productLabels;
          return labelArr.find((obj) => {
        ...
        const partnumbers = productArr
        .map((obj) => obj.value)
          parameters.partNumber=partnumbers;
          parameters.storeID=11;
          delete parameters.searchTerm;
